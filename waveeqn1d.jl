...
type WaveSolution1D
  ...
  # inner constructor
  function WaveSolution1D(n_x, dx, dt, c, d, bc_1, bc_2)
...
function step!(soln::WaveSolution1D)
...
function init_solution!(soln::WaveSolution1D, u_init::Array{Float64, 1},
...
function main()
  ...
  # let's try zero BCs
  # bc1(soln::WaveSolution1D) =  round(soln.t % 0.02 / 0.02)
  # bc1(soln::WaveSolution1D) = sin(1000 * soln.t) # + 0.5 * sin(2200 * soln.t)
  # bc1(soln::WaveSolution1D) = soln.c[1] * soln.u_x[1] * soln.dt + soln.u[1, soln.t_n2]
  bc2(soln::WaveSolution1D) = -soln.c[end] * soln.u_x[end] * soln.dt + soln.u[end, soln.t_n2]

  bc1(soln::WaveSolution1D) = 0
  # bc2(soln::WaveSolution1D) = 0

  # create solution storage
  soln = WaveSolution1D(n_x, dx, dt, c, d, bc1, bc2)
